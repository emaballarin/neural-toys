...
for i in range(0, Nsteps):
    ...
    diag_stop = random.gauss(0.0, math.sqrt(2)*sqside/3.0)

    for j in range(0, Npoints):
        x_axis = random.uniform(-2*diag_stop/math.sqrt(2), 2*diag_stop/math.sqrt(2))
        y_axis = random.uniform(-2*diag_stop/math.sqrt(2), 2*diag_stop/math.sqrt(2))
        point = [x_axis, y_axis]
